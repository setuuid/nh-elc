...
public class SysUserController {
    ...
    @GetMapping("/querySysUserAndDep")
    public Result<SysUser> querySysUserAndDep(){
        Result<SysUser> result = new Result<>();
        SysUser obj = (SysUser) SecurityUtils.getSubject().getPrincipal();
        SysUser sysUser = sysUserService.getById(obj.getId());
        result.setResult(sysUser);
        result.setSuccess(true);
        return result;
    }
    @RequestMapping(value = "/auditing", method = RequestMethod.PUT)
    public Result<SysUser> auditing(@RequestParam("userid") String userid) {
        Result<SysUser> result = new Result<SysUser>();
        try {
            ...
            if(sysUser==null) {
                ...
                sysUser.setAudit("2");
                sysUserService.auditing(sysUser);
                result.success("修改成功!");
            }
        } catch (Exception e) {
            e.printStackTrace();
     ...
     * 个人中心提交页
     */
    @PutMapping("/myselfUpdateUser")
    public Result<SysUser> myselfUpdateUser(@RequestBody SysUser sysUser){
        Result<SysUser> user = new Result<>();
    ...
    public Result<Boolean> checkPassword(@PathVariable("oldPassword") String oldPassword){
        Result<Boolean> stringResult = new Result<>();
        SysUser obj = (SysUser) SecurityUtils.getSubject().getPrincipal();
        String salt = obj.getSalt();
        String username = obj.getUsername();
        String password = obj.getPassword();
        String passwordEncode = PasswordUtil.encrypt(username,oldPassword,salt);
        if (password.equals(passwordEncode)){
            stringResult.setResult(true);
        }else {
            stringResult.setResult(false);
        }
        return stringResult;
    }
